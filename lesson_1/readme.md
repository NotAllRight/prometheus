...
Опис завдання:
Загорніть заголовки в правильні ```HTML<h>``` теги.

Головний заголовок – ```HTML<h1>```
Підзаголовок – ```HTML<h2>```
Другий і Третій – ```HTML<h3>```
Текст потрібно оформити як параграф - ```HTML<p>```
...
Загорнути у тег ```HTML<form>```
- Поле ```HTML<input>```, type=”text” placeholder=”Логін”;
- Поле ```HTML<input>```, type=”password” placeholder=”Пароль”;
- ```HTML<textarea>```, placeholder=”Ваше повідомлення”;
- Поле ```HTML<input>```, type=”submit” value=”Надіслати”;
...
Вимоги
- Заголовок в тег ```HTML<caption>```
- Заголовки колонок в тег ```HTML<thead>```
- Основна інформація в тег ```HTML<tbody>``` - два рядки
- Висновок в тег ```HTML<tfoot>``` - останній рядок, розмістити після ```HTML</tbody>```
...
Вимоги
HTML - жирним, але не ```HTML<b>```
Інтернеті - курсив, але не ```HTML<i>```
